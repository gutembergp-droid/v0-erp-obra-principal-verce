...
import { LayoutGrid, Users, DollarSign, Package, Cpu, MessageSquare, Settings } from "lucide-react"

const obraAdministrativoNavigation = [
  { name: "RH", href: "/obra/administrativo/rh", icon: Users },
  { name: "Financeiro", href: "/obra/administrativo/financeiro", icon: DollarSign },
  { name: "Patrimônio", href: "/obra/administrativo/patrimonio", icon: Package },
  { name: "TI", href: "/obra/administrativo/ti", icon: Cpu },
  { name: "Comunicação", href: "/obra/administrativo/comunicacao", icon: MessageSquare },
  { name: "Configurações", href: "/obra/administrativo/configuracoes", icon: Settings },
]

export function ObraAdministrativoNavbar() {
    ...
    <div className="flex items-end relative pb-0 px-[10px] overflow-hidden bg-gradient-to-b from-muted/20 to-transparent border-b border-border/40">
      <div className="w-[calc(100%-32px)] ml-4 px-[10px] overflow-hidden">
        <nav className="flex items-end justify-start gap-1 pt-3 pb-0 overflow-hidden relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {obraAdministrativoNavigation.map((item) => {
