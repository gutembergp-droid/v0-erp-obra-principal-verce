...
const obraNavigation = [
  { name: "Visão Geral", href: "/obra", icon: LayoutDashboard },
  { name: "Comercial", href: "/obra/comercial", icon: Receipt },
  { name: "Engenharia", href: "/obra/engenharia", icon: Wrench },
  { name: "Produção", href: "/obra/producao", icon: Hammer },
  { name: "Gerencial", href: "/obra/gerencial", icon: FileText },
  { name: "Garantidores", href: "/obra/garantidores", icon: Shield },
  { name: "Administrativo", href: "/obra/administrativo", icon: Users },
  { name: "Inteligência", href: "/obra/inteligencia", icon: Brain },
]

export function ObraNavbar() {
    ...
    <div className="flex items-end relative pb-0 px-[10px] overflow-hidden bg-gradient-to-b from-muted/20 to-transparent border-b border-border/40">
      <div className="w-[calc(100%-32px)] ml-4 px-[10px] overflow-hidden">
        <nav className="flex items-end justify-start gap-1 pt-3 pb-0 overflow-hidden relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {obraNavigation.map((item) => {
            const Icon = item.icon
            
            let isActive = false
            if (item.href === "/obra") {
              isActive = pathname === "/obra"
            } else {
