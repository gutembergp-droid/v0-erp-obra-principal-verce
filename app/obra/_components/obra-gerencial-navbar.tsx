...
import { LayoutDashboard, TrendingUp, Calendar, BookOpen, BarChart3 } from "lucide-react"

const obraGerencialNavigation = [
  { name: "Cockpit", href: "/obra/gerencial/cockpit", icon: LayoutDashboard },
  { name: "Indicadores", href: "/obra/gerencial/indicadores", icon: TrendingUp },
  { name: "Agenda", href: "/obra/gerencial/agenda", icon: Calendar },
  { name: "Encerramento", href: "/obra/gerencial/encerramento", icon: BookOpen },
]

export function ObraGerencialNavbar() {
    ...
    <div className="flex items-end relative pb-0 px-[10px] overflow-hidden bg-gradient-to-b from-muted/20 to-transparent border-b border-border/40">
      <div className="w-[calc(100%-32px)] ml-4 px-[10px] overflow-hidden">
        <nav className="flex items-end justify-start gap-1 pt-3 pb-0 overflow-hidden relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {obraGerencialNavigation.map((item) => {
