...
import { LayoutGrid, FileText, TrendingUp, Users, Wrench, Calendar, Target, Activity, MapPin, Package, BarChart3, CheckSquare } from "lucide-react"

const obraProducaoNavigation = [
  { name: "Visão Geral", href: "/obra/producao", icon: LayoutGrid },
  { name: "RDO", href: "/obra/producao/rdo", icon: FileText },
  { name: "Produtividade", href: "/obra/producao/produtividade", icon: TrendingUp },
  { name: "Apontamentos", href: "/obra/producao/apontamentos", icon: Users },
  { name: "Equipamentos", href: "/obra/producao/equipamentos", icon: Wrench },
  { name: "Apropriação", href: "/obra/producao/apropriacao", icon: Calendar },
  { name: "PDS", href: "/obra/producao/pds", icon: Target },
  { name: "Aferição", href: "/obra/producao/afericao", icon: Activity },
  { name: "Campo", href: "/obra/producao/campo", icon: MapPin },
  { name: "Diário", href: "/obra/producao/diario", icon: Package },
  { name: "EAP", href: "/obra/producao/eap", icon: BarChart3 },
  { name: "Fechamento", href: "/obra/producao/fechamento", icon: CheckSquare },
]

export function ObraProducaoNavbar() {
    ...
    <div className="flex items-end relative pb-0 px-[10px] overflow-hidden bg-gradient-to-b from-muted/20 to-transparent border-b border-border/40">
      <div className="w-[calc(100%-32px)] ml-4 px-[10px] overflow-hidden">
        <nav className="flex items-end justify-start gap-1 pt-3 pb-0 overflow-hidden relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {obraProducaoNavigation.map((item) => {
            const Icon = item.icon
            const isActive = pathname === item.href || (item.href !== "/obra/producao" && pathname?.startsWith(item.href + "/"))
