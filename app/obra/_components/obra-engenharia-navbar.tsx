...
import { LayoutGrid, FileText, Folder, GitBranch, Calendar } from "lucide-react"

const obraEngenhariaNavigation = [
  { name: "Visão Geral", href: "/obra/engenharia", icon: LayoutGrid },
  { name: "Projetos", href: "/obra/engenharia/projetos", icon: FileText },
  { name: "Documentos", href: "/obra/engenharia/documentos", icon: Folder },
  { name: "Revisões", href: "/obra/engenharia/revisoes", icon: GitBranch },
  { name: "Planejamento", href: "/obra/engenharia/planejamento", icon: Calendar },
]

export function ObraEngenhariaNavbar() {
    ...
    <div className="flex items-end relative pb-0 px-[10px] overflow-hidden bg-gradient-to-b from-muted/20 to-transparent border-b border-border/40">
      <div className="w-[calc(100%-32px)] ml-4 px-[10px] overflow-hidden">
        <nav className="flex items-end justify-start gap-1 pt-3 pb-0 overflow-hidden relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {obraEngenhariaNavigation.map((item) => {
            const Icon = item.icon
            const isActive = pathname === item.href || (item.href !== "/obra/engenharia" && pathname?.startsWith(item.href + "/"))
