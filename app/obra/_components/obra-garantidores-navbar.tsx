...
import { LayoutGrid, HardHat, Scale, TreePine, Shield } from "lucide-react"

const obraGarantidoresNavigation = [
  { name: "Visão Geral", href: "/obra/garantidores", icon: LayoutGrid },
  { name: "SSMA", href: "/obra/garantidores/ssma", icon: HardHat },
  { name: "Jurídico", href: "/obra/garantidores/juridico", icon: Scale },
  { name: "Meio Ambiente", href: "/obra/garantidores/meio-ambiente", icon: TreePine },
  { name: "Qualidade", href: "/obra/garantidores/qualidade", icon: Shield },
]

export function ObraGarantidoresNavbar() {
    ...
    <div className="flex items-end relative pb-0 px-[10px] overflow-hidden bg-gradient-to-b from-muted/20 to-transparent border-b border-border/40">
      <div className="w-[calc(100%-32px)] ml-4 px-[10px] overflow-hidden">
        <nav className="flex items-end justify-start gap-1 pt-3 pb-0 overflow-hidden relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {obraGarantidoresNavigation.map((item) => {
            const Icon = item.icon
            const isActive = pathname === item.href || (item.href !== "/obra/garantidores" && pathname?.startsWith(item.href + "/"))
