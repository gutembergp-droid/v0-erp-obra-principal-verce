...
const obraComercialNavigation = [
  { name: "Visão Geral", href: "/obra/comercial", icon: LayoutGrid },
  { name: "Estruturação", href: "/obra/comercial/estruturacao", icon: FileText },
  { name: "Custos", href: "/obra/comercial/custo", icon: DollarSign },
  { name: "Receitas", href: "/obra/comercial/receita", icon: TrendingUp },
  { name: "Suprimentos", href: "/obra/comercial/suprimentos", icon: ShoppingCart },
  { name: "Medições", href: "/obra/comercial/medicao-producao", icon: Package },
  { name: "Comparativo", href: "/obra/comercial/comparativo", icon: GitCompare },
  { name: "Desvios", href: "/obra/comercial/analise-desvios", icon: AlertTriangle },
  { name: "Metas", href: "/obra/comercial/custo-meta", icon: Target },
  { name: "Analytics", href: "/obra/comercial/analytics", icon: BarChart3 },
]

export function ObraComercialNavbar() {
    ...
    <div className="flex items-end relative pb-0 px-[10px] overflow-hidden bg-gradient-to-b from-muted/20 to-transparent border-b border-border/40">
      <div className="w-[calc(100%-32px)] ml-4 px-[10px] overflow-hidden">
        <nav className="flex items-end justify-start gap-1 pt-3 pb-0 overflow-hidden relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {obraComercialNavigation.map((item) => {
            const Icon = item.icon
            const isActive = pathname === item.href || (item.href !== "/obra/comercial" && pathname?.startsWith(item.href + "/"))
