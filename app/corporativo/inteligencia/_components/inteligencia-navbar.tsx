...
import { Users2, Factory, MessageSquare, GraduationCap } from "lucide-react"

const inteligenciaNavigation = [
  { name: "Comitê de IAs", href: "/corporativo/inteligencia/comite", icon: Users2 },
  { name: "Fábrica de IAs", href: "/corporativo/inteligencia/fabrica", icon: Factory },
  { name: "Hermes (Conversa)", href: "/corporativo/inteligencia/hermes", icon: MessageSquare },
  { name: "Treinamento", href: "/corporativo/inteligencia/treinamento", icon: GraduationCap },
]

export function InteligenciaNavbar() {
    ...
    <div className="flex items-end relative pb-0 px-[10px] overflow-hidden bg-gradient-to-b from-muted/20 to-transparent border-b border-border/40">
      <div className="w-[calc(100%-32px)] ml-4 px-[10px] overflow-hidden">
        <nav className="flex items-end justify-start gap-1 pt-3 pb-0 overflow-hidden relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {inteligenciaNavigation.map((item) => {
