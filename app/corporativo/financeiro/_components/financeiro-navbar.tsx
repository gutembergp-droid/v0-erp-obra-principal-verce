...
const financeiroNavigation = [
  { name: "Visão Geral", href: "/corporativo/financeiro", icon: LayoutDashboard },
  ...
  { name: "DRE", href: "/corporativo/financeiro/dre", icon: BarChart3 },
  { name: "Conciliação", href: "/corporativo/financeiro/conciliacao", icon: FileText },
  { name: "Orçamento", href: "/corporativo/financeiro/orcamento", icon: PiggyBank },
  { name: "Relatórios", href: "/corporativo/financeiro/relatorios", icon: FileText },
]

export function FinanceiroNavbar() {
      ...
      <div className="w-[calc(100%-32px)] ml-4 px-[10px] overflow-hidden">
        <nav className="flex items-end justify-start gap-1 pt-3 pb-0 overflow-hidden relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {financeiroNavigation.map((item) => {
            ...
            if (item.href === "/corporativo/financeiro") {
              // Para "Visão Geral": APENAS match exato
              isActive = pathname === "/corporativo/financeiro"
            } else {
