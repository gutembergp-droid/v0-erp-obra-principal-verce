...
const consoleNavigation = [
  { name: "Visão Geral", href: "/corporativo/console", icon: LayoutDashboard },
  { name: "Usuários", href: "/corporativo/console/usuarios", icon: Users },
  { name: "Perfis", href: "/corporativo/console/perfis", icon: Shield },
  { name: "Permissões", href: "/corporativo/console/permissoes", icon: Key },
  { name: "Alçadas", href: "/corporativo/console/alcadas", icon: Network },
  ...
  { name: "Convites", href: "/corporativo/console/convites", icon: Mail },
  { name: "Suporte", href: "/corporativo/console/suporte", icon: Headphones },
  { name: "Integrações", href: "/corporativo/console/integracoes", icon: Plug },
  { name: "Parâmetros", href: "/corporativo/console/parametros", icon: Settings },
  { name: "Layouts", href: "/corporativo/console/layouts-templates", icon: FileText },
]
      ...
      <div className="w-[calc(100%-32px)] ml-4 px-[10px] overflow-hidden">
        <nav className="flex items-end justify-start gap-1 pt-3 pb-0 overflow-hidden relative z-10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {consoleNavigation.map((item) => {
            ...
            // Lógica robusta para detecção de rota ativa
            let isActive = false
            
            if (item.href === "/corporativo/console") {
              // Para "Visão Geral": APENAS match exato
              isActive = pathname === "/corporativo/console"
            } else {
              // Para outras páginas: match exato OU começar com a rota + /
            ...
            return (
              ...
              >
                ...
                {/* Camuflador da linha horizontal - cria conexão perfeita */}
                {isActive && (
                  <div 
